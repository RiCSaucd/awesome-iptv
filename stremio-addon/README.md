...
By default it loads the public [iptv-org](https://github.com/iptv-org/iptv) playlist (~11,000
channels across 30 genres). Point it at your own playlist — a URL or a local file — with
`IPTV_PLAYLIST_URL`:
...
On a network that blocks `iptv-org.github.io`, the same file is served from the project's
`gh-pages` branch:

```sh
IPTV_PLAYLIST_URL=https://raw.githubusercontent.com/iptv-org/iptv/gh-pages/index.m3u npm start
```
...
A `group-title` often lists several genres separated by semicolons — iptv-org writes
`group-title="Animation;Classic;Movies"`. Each becomes a separate genre, so a channel is reachable
under every genre it carries. Commas are left alone, since they belong to a genre's own name
(`News, Talk`).
